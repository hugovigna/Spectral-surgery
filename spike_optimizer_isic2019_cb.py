...
class SpikeOptimizer:
    ...
    def run(self):
        ...
        # Val baseline pour suivi de généralisation (pas utilisée pour les décisions).
        ...
        # EMA windows τ1 = n_iter/4 (mean), τ2 = n_iter (variance) —
        # principled scaling to the actual iteration budget.
        n_it    = max(int(cfg["n_iter"]), 2)
        β1_a    = 1.0 - 4.0 / n_it
        β2_a    = 1.0 - 1.0 / n_it
        ε_a     = 1e-8

        # Best-on-val tracking (critère : val std minimal, but matches SS objective)
        ...
        for it in range(1, cfg["n_iter"] + 1):
            ...
            # Eval val (real imbalanced) — pas utilisée pour les décisions, juste suivi.
            ...
            # Best-on-val parallèle (critère : val std minimal — aligné avec objectif SS)
            if not rolled_back and val_std < best_val_std and self.cfg.get("save_model"):
                best_val_std = val_std
                best_val_iter = it
                best_val_path = self.cfg["model_out"].replace(".keras", "_bestval.keras")
...
if __name__ == "__main__":
    ...
    # HVP samples — balancés par classe (n_hvp/N_CLASSES par classe) → estime
    # la Hessienne de la loss balancée plutôt que de la loss naturelle.
    # Avec n_hvp=256 et N_CLASSES=8 : 32/classe.
    n_per_class = CONFIG["n_hvp_samples"] // N_CLASSES
    hvp_idx = []
    for c in range(N_CLASSES):
        cls_idx = np.where(y_train == c)[0]
        hvp_idx.append(rng.choice(cls_idx, n_per_class, replace=False))
    hvp_idx = np.concatenate(hvp_idx)
    rng.shuffle(hvp_idx)
    x_hvp   = x_train[hvp_idx]
    y_hvp   = y_train[hvp_idx]
    print(f"    HVP samples : {len(x_hvp)} ({n_per_class}/classe, balancé)")
